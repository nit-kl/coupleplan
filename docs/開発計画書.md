...
| 領域 | ギャップ | 主な扱い |
|------|----------|----------|
| 永続化 | **D1 スキーマ＋D1 リポジトリ**（[0005](./adr/0005-database-d1.md)）。初版に加え `0002_refresh_sessions.sql`。**ローカル**は従来どおり Node+インメモリ | ルーレット/ニンジャの実データ層を P1 で接続 |
...
| T3 | DB | 完了 | Cloudflare D1（[0005](./adr/0005-database-d1.md)）。SQL は `apps/api/migrations/0001_initial.sql` と `0002_refresh_sessions.sql` |
...
ルーレット/ニンジャ前に、運用・認証まわりで次を追加済み（索引は [adr/README.md](./adr/README.md)）。
